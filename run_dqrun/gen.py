...
STR32_COLUMN_NUM = 10
STR64_COLUMN_NUM = 10
COLUMN_NUM = UINT64_COLUMN_NUM + STR32_COLUMN_NUM + STR64_COLUMN_NUM + 1 
Row = List
...
def gen_column_names(column_num: int) -> List[str]:
    column_names = [f"c{i}" for i in range(column_num)]
    column_names.append("time")
    return column_names

# def gen_column_types(column_num: int) -> List[str]:
#     column_types = [
#         "uint64" if i < UINT64_COLUMN_NUM else 
#         "str32" if i < UINT64_COLUMN_NUM + STR32_COLUMN_NUM else 
#         "str64" for i in range(column_num-1)
#     ]
...
def type_to_sql(column_type: str) -> str:
    if column_type == "uint64":
        return "Uint64"
    elif column_type == "str32" or column_type == "str64":
        return "String"
    elif column_type == "Datetime":  
        return "Uint64"
    else:
        raise RuntimeError(f"Unsupported or unexpected column type: {column_type}")
...
#     elif column_type == "str64":
#         return int_to_str(value, 64)
#     elif column_type == "str" and row_index == COLUMN_NUM - 1:
#         return f"{row_index % 24:02}{row_index % 60:02}{row_index % 60:02}"
#     else:
#         raise RuntimeError()
